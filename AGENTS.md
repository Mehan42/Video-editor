...
- `internal/cache` — локальный run-кэш под `OutputRoot/.cache/`, ключ = `sha256(input) + ParamsHash(ffmpeg, whisper, model, language, chunk chars, overlap)`. Hit копирует аудио+транскрипт+srt в новый runRoot, manifest переписывается с новым run id. Флаг `--no-cache` отключает. Любая ошибка чтения/валидации трактуется как miss, не как падение.
