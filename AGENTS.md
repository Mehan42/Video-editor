...
- Бинарники ffmpeg и whisper.cpp в gitignore, ожидаются по путям `ffmpeg\bin\ffmpeg.exe`, `whisper.cpp\bin\whisper-cli.exe`, `whisper.cpp\models\ggml-base.bin`. Не коммить их. Пути переопределяются флагами `--ffmpeg`/`--whisper`/`--model`. Опциональный URL-ingress через `tools\yt-dlp.exe` (`--ytdlp` для альтернативы; каталог `tools/` в gitignore).
...
- `internal/cache` — локальный run-кэш под `OutputRoot/.cache/`, ключ = `sha256(input) + ParamsHash(ffmpeg, whisper, model, language, chunk chars, overlap)`. Hit копирует аудио+транскрипт+srt в новый runRoot, manifest переписывается с новым run id. Флаг `--no-cache` отключает. Любая ошибка чтения/валидации трактуется как miss, не как падение. **Кэш пропускается при URL-входе** (source_url задан — удалённый ресурс мог измениться).
- `internal/download` — opt-in yt-dlp ingress для http(s) URL. Запускается только при явном `--allow-download`; конфиг `--ytdlp` (по умолчанию `tools\yt-dlp.exe`). Использует `exec.CommandContext` с раздельными аргументами (никогда не строкой), передаёт pipeline ffmpeg через `--ffmpeg-location` для merge DASH; после успеха ищет единственный `download.*` в dstDir, валидирует размер по `--max-input-bytes`, отказывается от symlink-выхода за пределы staging-каталога. URL игнорирует локальный кэш.
- `internal/chunk` — детерминированный chunker (одинаковый вход → одинаковый выход).
- `internal/llm` — loopback-only адаптер Bionic (LM Studio): готовность через `/v1/models`, лимит размера ответа. Chat по умолчанию заблокирован (`ErrEgressBlocked`); для summary включение идёт ТОЛЬКО через явный операторный флаг `process --enable-summary`. Роль сообщения — закрытое множество (`system|user|assistant`), недоверенный контент (транскрипт/chunks) отправляется только как `user` через `NewUserMessage`; провайдер/endpoint/капабилити задаётся в `Config` при конструировании и не может быть выбран из текста сообщения (`TestChatRequestConfigIsolation`). Тесты (`client_test.go`, `summarize_test.go`, только httptest) покрывают: loopback-валидацию, default-блокировку chat, malformed JSON, пустой `data`, HTTP-ошибку провайдера, превышение `MaxResponseBytes`, timeout, пустые `choices`, malformed chat-ответ, разделение системной политики и пользовательского контента в summary-запросе, лимит размера транскрипта.
